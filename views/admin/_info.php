...
<?php $this->beginContent('@dsanchez98/user/views/admin/update.php',
                           ['user' => $user]) ?>

<table class="table">
    <tr>
        <td><strong><?= Yii::t('user', 'Registration time') ?>:</strong></td>
        <td><?= Yii::t('user', '{0, date, MMMM dd, YYYY HH:mm}',
                        [$user->created_at]) ?></td>
    </tr>
    <tr>
        <td><strong><?= Yii::t('user', 'Ultimo ingreso') ?>:</strong></td>
        <td><?= Yii::t('user', '{0, date, MMMM dd, YYYY HH:mm}',
                        [$user->last_login]) ?></td>
    </tr>
<?php if ($user->registration_ip !== null): ?>
    ...
    <tr>
        ...
        <?php if ($user->isConfirmed): ?>
            <td class="text-success"><?= Yii::t('user',
                                             'Confirmed at {0, date, MMMM dd, YYYY HH:mm}',
                                             [$user->confirmed_at]) ?></td>
<?php else: ?>
    ...
    <tr>
        ...
        <?php if ($user->isBlocked): ?>
            <td class="text-danger"><?= Yii::t('user',
